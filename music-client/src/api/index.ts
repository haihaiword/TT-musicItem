...
const HttpManager = {
  //返回排行榜
  GetToplist: () => post(`TopList`),
  // 修改密码
  changePassword: ({email,ConfirmWorld,newPassword,ConfirmNewPassword}) => post(`user/changePassword`, {email,ConfirmWorld,newPassword,ConfirmNewPassword}),
  // 获取图片信息
  ...
  // 注册
  SignUp: ({username,password,sex,phoneNum,email,birth,introduction,location,code}) => post(`user/add`, {username,password,sex,phoneNum,email,birth,introduction,location,code}),
  // 删除用户
